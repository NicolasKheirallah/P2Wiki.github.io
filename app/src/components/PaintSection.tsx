  ...
  return (
    <div className="overflow-x-auto scrollbar-hide -mx-2 px-2">
      <table className="w-full min-w-[900px] border-collapse">
        <thead className="sticky top-14 z-30 border-b border-[var(--ps-table-border)]">
          <tr>
            <th className="sticky left-0 bg-[var(--ps-bg)] text-left py-3 pr-4 text-[12px] font-normal uppercase tracking-wider w-[40px] z-40" style={{ color: 'var(--ps-text-tertiary)' }}>
              {t('swatch')}
            </th>
            <th className="sticky left-[40px] bg-[var(--ps-bg)] text-left py-3 pr-4 text-[12px] font-normal uppercase tracking-wider w-[160px] z-40 border-r border-[var(--ps-border-light)] shadow-[2px_0_5px_-2px_rgba(0,0,0,0.1)]" style={{ color: 'var(--ps-text-tertiary)' }}>
              {t('colour')}
            </th>
            <th className="text-left py-3 pr-4 text-[12px] font-normal uppercase tracking-wider w-[180px] bg-[var(--ps-bg)]" style={{ color: 'var(--ps-text-tertiary)' }}>
              {t('volvoEquivalent')}
            </th>
            <th className="text-left py-3 pr-4 text-[12px] font-normal uppercase tracking-wider w-[100px] bg-[var(--ps-bg)]" style={{ color: 'var(--ps-text-tertiary)' }}>
              {t('code')}
            </th>
            {modelYearLabels.map((label, i) => {
              ...
              return (
                <th
                  key={i}
                  className="text-center py-3 px-1 text-[12px] font-normal uppercase tracking-wider cursor-pointer transition-colors duration-150 min-w-[70px] bg-[var(--ps-bg)] hover:bg-[var(--ps-bg-secondary)]"
                  style={{
